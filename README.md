...
7. $n$个结点的二叉树,最小高度是$\downarrow log_{2}n \downarrow + 1$,最大高度是$n$
...
* 散列表: 一种数据结构,能够根据数据元素的关键字计算出在散列表中的存储地址
* 散列函数: 能够计算出数据元素和存储地址的映射关系
* 理想情况下,查找元素时间复杂度可以到$O(1)$
* 冲突(碰撞): 位置已经存储了其他元素,构建更好的散列函数来减少冲突
* 同义词: 不同关键字映射到同一个存储地址,就是同义词
...
* 选择数码分布较为均匀的若干位作为散列地址
* 适用于关键字集合已知,而且关键字某几个数码位分布均匀

平方取中法

* 选择关键字的平方值的中间几位来作为散列地址
* 适用于关键字的每位取值都不均匀的场景

##### 拉链法

* 处理冲突的方法之一
* 插入操作: 计算散列地址(例如使用除留余数法),插入元素时冲突元素使用链表连接,即拉链法,插入时使用头插法(默认)
* 查找操作: 计算散列地址(例如使用除留余数法),对比链表找到元素
* 删除操作: 先查找再删除
* 查找长度: 指的是对比关键字的次数(默认空指针的对比不统计在内)
* 优化: 在插入时,保证链表有序,这样可以提高查找效率

##### 开放定址法

* 处理冲突的方法之一
* 原理: 如果冲突就重新寻找一个新的空闲位置插入,关键点在于如何寻找空闲位置(偏移量$d_i$)
* 有四种细分方法
  * $H_i=(H(key)+d_i)\%m$
  * $d_0=0$
  * $d_i$的$i$不超过$m-1$
* 插入操作: 冲突时根据$d_i$找到下一个位置继续尝试插入
* 查找操作: 第一次查找不匹配后,还会根据$d_i$继续查找匹配,直到最终成功或者失败
* 删除操作: 先查找再删除
...
问题
* 问题1: 删除时会出现空位,从而导致后面的查找操作失败,应该使用逻辑删除而不是物理删除
* 问题2: 逻辑删除过多时,散列表看起来很满,实际上很空,需要定期整理(物理清空)
* 问题3: 若$m$能表示为$4j+3$的质数,平方探测法才能探测到所有位置
* 问题4: 若双散列法的第二个散列函数计算的值和$m$互质,能保证探测到所有位置,若$m$本身就是质数,则第二个散列函数可以设计为$m-(key\%m)$
* 问题5: 若伪随机序列人为设计的好,则也能探测到所有位置
...
* 查找成功时: 每个关键字查找长度汇总,除以关键字的个数,查找成功是看关键字的数量,逻辑删除的关键字也需要统计在内
* 查找失败时: 每个初始散列地址查找失败的长度汇总,除以初始散列地址的个数(而不是除以散列表长度,也不是关键字总数),而且逻辑删除元素地址也统计到失败ASL的计算了,查找失败是看散列函数的可能性
* 逻辑删除位不同于空位置,逻辑删除位不代表查找失败
* ASL和装填因子$α$相关,不直接依赖于已有关键字个数$n$或者散列表长度$m$

装填因子

* $装填因子α=\frac{表的记录数n}{表的长度m}$
* 反映了散列表的填满的程度,装填因子越大,越满,越容易冲突,从而导致插入查找效率降低,ASL增加

聚集(堆积)现象

* 由于有一堆关键字在散列表中是聚集在一起的,这样就导致当冲突发生时,几个初始地址不同的关键字会争夺同一个后继的散列地址,这种现象叫做聚集现象
* 会导致查找插入效率下降,ASL增加
* 使用平方探测法代替线性探测法减少聚集现象
...
* 需要关注时间复杂度和空间复杂度
* 还需要关注算法稳定性: 相同元素是否发生排序交换,不发生排序交换则称之为稳定
* 稳定一定比不稳定好?不一定,看实际需求
* 内部排序和外部排序: 是否全部在内存中进行排序

### 插入排序

* 每次将元素插入到局部有序的子序列
* 包含直接插入排序,折半插入排序,希尔排序

##### 直接插入排序

* 时间复杂度: 最坏平均$O(n^2)$,最好情况$O(n)$
* 空间复杂度: $O(1)$
* 算法稳定,算法简单
* 待排序的元素序列基本有序情况下,选择插入排序,更好
* 可能出现这种情况: 在最后一趟排序开始之前,所有的元素并没有出现在最终的位置上
* 最坏情况下比较次数: $\frac{n(n-1)}{2}$
* 经过两趟排序后,前三个元素应该是局部有序的

##### 折半插入排序

* 时间复杂度: $O(n^2)$
* 比较关键字的次数减少了,但是移动关键字的次数没变
* 注意是顺序表存储
...
* 基于插入排序的进一步优化,将顺序表按照$d$间隔进行划分,划分为不同的子表(例如d=4,则索引0和索引4是同一个子表),针对每一个子表进行直接插入排序,然后继续缩短$d$间隔,重复操作直到$d=1$
* $d$间隔的划分,初始时$d=\frac{n}{2}$
* $d$缩短间隔,指的是每次都除以2,即缩小一半
* 无法用数学手段证明确切的时间复杂度,但是优于插入排序
* 算法不稳定
* 仅适用于顺序表,不适用于链表
...
* 时间复杂度: 最坏平均$O(n^2)$,最好情况$O(n)$
* 空间复杂度: $O(1)$
* 最好情况$n-1$次对比,$0$次交换,本身有序
* 最坏情况$\frac{n(n-1)}{2}$次对比,$\frac{n(n-1)}{2}$次交换,逆序情况
* 注意交换次数和移动次数不同,移动次数=3*交换次数
* 算法稳定,算法简单
* 适用于链表
* 经过两趟排序后,应该有两个最大或者最小放在最终位置
* 每趟都能确定一个元素在其最终位置
* 能形成全局有序的子序列

##### 快速排序

* 时间复杂度: 最好平均$O(nlog_{2}n)$,最坏情况$O(n^2)$,partition方法部分的时间复杂度是$O(n)$,而quicksort方法部分的时间复杂度和递归深度相关,也就是二叉树的最小高度,是$O(log_{2}n)$,最坏情况是本身有序的情况,枢轴元素的选取不均匀导致
* 空间复杂度: $O(log_{2}n)$,空间复杂度也和递归深度相关
* 算法不稳定,算法复杂
* 优化:头中尾的中间值作为枢轴元素,或者随机选一个元素作为枢轴元素
* 每趟都能确定一个元素在其最终位置(这里把一趟理解为一次划分),每一次划分n个元素需要比较n-1次
* 能确定枢轴(基准)元素的最终位置
* 注意区分408题目中一趟排序和一次划分的区别,一趟排序可能确定多个枢轴元素位置,一次划分只能确定一个枢轴元素的位置

### 选择排序

* 每一趟选择最大或最小关键字的元素并加入有序子序列
* 包含简单选择排序,堆排序
...
* 完全二叉树结点i的左孩子是$2i$
* 完全二叉树结点i的右孩子是$2i+1$
* 完全二叉树结点i的父结点是$\downarrow \frac{i}{2} \downarrow$
* 完全二叉树结点i所在高度是$\lceil log_{2}(n+1) \rceil$或者$\downarrow log_{2}n \downarrow + 1$
* n个结点若 $i>\downarrow \frac{n}{2} \downarrow$,则是叶子结点,反之则是分支结点,意味着分支结点存储在顺序数组的前面部分
* 完全二叉树按照层序遍历,存储到顺序表数组中

大根堆
* 完全二叉树中,根结点值大于左孩子和右孩子,不同于二叉排序树

小根堆
* 完全二叉树中,根结点值小于左孩子和右孩子

大根堆的建立

* 思路: 检查非终端结点是否满足大根堆,不满足则进行调整,调整方式是将结点和更大的孩子结点互换,调整后可能继续向下调整(小元素不断下坠),详见代码

堆排序

* 思路: 初始化大根堆,然后交换首尾元素,让大值跑到数组后面,然后不断循环调用创建大根堆的方法,不断交换首尾元素,直到结束,详见代码
* 时间复杂度: 最好平均最坏$O(nlog_{2}n)$ 
* 空间复杂度: $O(1)$
* 算法不稳定,算法复杂
* 每趟都能确定一个元素在其最终位置
* 能形成全局有序的子序列 
...
* 一个结点,每下坠一层,只需要最多对比关键字2次(左右孩子对比一次,结点和大的孩子对比一次)
* 若树高度h,结点在第i层,则该结点最多下坠$h-i$层,关键字对比次数最多$2(h-i)$
* 最后一层不需要调整
* 由于是完全二叉树,所以有$\lceil log_{2}(n+1) \rceil$或者$\downarrow log_{2}n \downarrow + 1$
* 根据二叉树性质,第i层节点数量是$2^{i-1}$
* 综上可知每一层的调整次数求和得:$1*2(h-1)+2^1*2(h-2)+2^2*2(h-3)... <= 4n$
* 所以建堆过程时间复杂度$O(n)$

堆排序过程的时间复杂度分析
* 初始建堆后,执行for循环的调整大根堆的方法,执行$n-1$趟
* 根据$\lceil log_{2}(n+1) \rceil$或者$\downarrow log_{2}n \downarrow + 1$,每一趟的时间复杂度不超过$O(h)=O(log_{2}n)$
* $n-1$趟不超过$O(nlog_{2}n)$
* 总体时间复杂度$O(n)+O(nlog_{2}n)=O(nlog_{2}n)$,空间复杂度$O(1)$

插入堆
* 插入表尾(堆底)
* 向上调整,和父节点对比大小即可,一直到无法继续为止
* 时间复杂度: 比较次数等于树的高度减一,即$O(log_{2}n)$

删除堆
* 删除元素,然后使用表尾(堆底)的元素来顶替空位
* 然后一直下坠,一直到无法下坠为止
* 时间复杂度: 比较次数等于树的高度减一,即$O(log_{2}n)$

### 归并排序

* 把两个或者多个有序序列合并成一个,考研中要求二路合并,也有多路归并
* 时间复杂度: 最好最坏平均: $O(nlog_{2}n)$ 
* 空间复杂度: 平均最坏$O(n)$,因为需要辅助数组b
* 算法稳定,算法复杂
* $m$路归并,每选出一个元素,需要对比关键字次数$m-1$
* 每一趟的归并的时间复杂度在$O(n)$,$h$高度的归并树,执行$h-1$趟,又因为$n<=2^{h-1}$,可得$h-1=\lceil log_{2}n \rceil$,所以趟数又等于$\lceil log_{2}n \rceil$,总共的时间复杂度就是$O(nlog_{2}n)$
* 每一趟的归并的时间复杂度在$O(n)$,是因为$n$个关键字对比次数$<=n-1$
* 当一个序列的最小值都比另一个序列最大值还大时,比较次数是n次,当两个序列间隔着比较时,比较次数是最多的,达到$2n-1$

### 基数排序

* 思路
  * 设置r个空的辅助队列
  * 第一趟分配,按照关键字个位,分配到不同队列,注意不足位补0然后比较
  * 第一趟收集,如果要递减序列,则从个位大的收集到个位小的,靠近队头的排在链表前面,最终得到个位递减的序列
  * 第二趟分配,按照关键字十位,分配到不同队列,由于是基于第一趟,个位越大的越靠近队头
  * 第二趟收集,相同,按照十位进行收集
  * 第三趟分配,按照关键字百位,分配到不同队列,由于基于第二趟的结果,所以十位越大越靠近队头
  * 第三趟收集,相同,按照百位进行收集
* 基数:r,例如思路中针对三位数字的排序,r就是10,代表有十种不同取值
* 大多是基于链表存储来实现
* 时间复杂度: 每一趟分配需要$O(n)$,收集一个r队列元素只需要$O(1)$常数,所以每一趟的收集$O(r)$,一共d趟,总计的时间复杂度是$O(d(n+r))$
* 空间复杂度: $O(r)$,和辅助队列相关
* 算法稳定
* 反映了一个思路,就是优先级越高的关键字越应该放在后面进行排序,这里最后才排序百位,因为百位对于排序优先级最高

基数排序的应用场景
* 例如所有学生按照年月日的生日信息进行排序
* 适用:d小,r小,n大

### 计数排序

* 思路:想要确定某个元素的排序位置,就统计比这个元素小的个数,计数排序
  * 创建辅助数组,遍历数组然后统计各个元素出现的次数,记录在辅助数组中
  * 基于之前结果,再次统计辅助数组,让辅助数组的值去代表小于等于该元素的个数
  * 基于之前的结果,进行排序,还需要一个和原数组大小相同的新的辅助数组:选一个待排序元素,然后看他在辅助数组的值,先把这个值减少一,再把这个元素插入到新的辅助数组的对应位置
* 空间复杂度: $O(n+k)$,n是数组长度,k是辅助数组长度
* 时间复杂度: 四个循环相加$O(n+k)$,如果$k=O(n)$,则是$O(n)$,此时优于快速排序,堆排序,如果$k>O(nlog_{2}n)$,则不优于快速排序,堆排序
* 待排序元素需要是整数类型,取值范围0到k,k较小,这些条件是适用条件(学习考试时)
...
<table>
    <thead>
        <tr>
            ...
            <th colspan="2">空间复杂度</th>
            <th rowspan="2">是否稳定</th>
            <th rowspan="2">存储结构</th>
            <th rowspan="2">初始序列和趟数</th>
            <th rowspan="2">每趟情况</th>
            <th rowspan="2">备注</th>
        </tr>
        <tr>
            <th>最好情况</th>
    ...
    <tbody>
        <tr>
            ...
            <td>/</td>
            <td>是</td>
            <td>顺序存储,链式存储;插入方法链式存储更有优势(无需大量移动)</td>
            <td>初始有序时比较次数最少(n-1),但是趟数和初始状态无关(n-1趟)</td>
            <td>前n趟能确定n+1个元素有序</td>
            <td>n较小,记录本身数据量小;或初始基本有序</td>
        </tr>
        <tr>
            <td>折半插入排序</td>
            <td>/</td>
            ...
            <td>/</td>
            <td>顺序存储</td>
            <td>初始序列状态和比较次数无关,不同初始序列比较次数最多相差1次(认为是相同的)</td>
            <td>前n趟能确定n+1个元素有序</td>
            <td>/</td>
        </tr>
        <tr>
            ...
            <td>/</td>
            <td>否</td>
            <td>顺序存储;顺序存储能够更快找到间隔d的元素(随机存储)</td>
            <td>/</td>
            <td>只是保证子序列有序不能保证处于最终位置上(不同于插入排序)</td>
            <td>/</td>
        </tr>
        <tr>
            <td>冒泡排序</td>
            ...
            <td>是</td>
            <td>顺序存储,链式存储</td>
            <td>时间复杂度和初始状态相关,最好情况比较n-1次(一趟)结束</td>
            <td>每趟都能确定一个元素在其最终位置(min,max)</td>
            <td>初始基本有序</td>
        </tr>
        <tr>
            <td>快速排序</td>
            ...
            <td>否</td>
            <td>顺序存储</td>
            <td>时间复杂度和初始状态相关,初始有序的情况下时间复杂度最大,排序趟数最坏情况下n-1趟</td>
            <td>每趟都能确定一个元素在其最终位置</td>
            <td>n较大,算法不稳定,时间复杂度低;并行执行</td>
        </tr>
        <tr>
            <td>简单选择排序</td>
            <td>$O(n^2)$</td>
            ...
            <td>否</td>
            <td>顺序存储,链式存储</td>
            <td>时间复杂度和初始序列无关,比较次数固定,排序趟数也是固定的n-1趟</td>
            <td>每趟都能确定一个元素在其最终位置(min,max)</td>
            <td>n较小,记录本身数据量大</td>
        </tr>
        <tr>
            ...
            <td>/</td>
            <td>否</td>
            <td>顺序存储;顺序存储能够保证快速找到左右孩子元素(随机存储)</td>
            <td>初始状态和时间复杂度无关,因为一定需要n-1趟的调整;比较次数和初始序列状态有关</td>
            <td>每趟都能确定一个元素在其最终位置(min,max)</td>
            <td>n较大,算法不稳定,时间复杂度低,无最坏情况;并行执行(有争议)</td>
        </tr>
        <tr>
            <td>二路归并排序</td>
            <td>$O(nlog_{2}n)$</td>
            ...
            <td>是</td>
            <td>顺序存储,链式存储</td>
            <td>排序趟数和初始序列无关;比较次数和初始序列相关</td>
            <td>/</td>
            <td>n较大,算法稳定,时间复杂度低;并行执行</td>
        </tr>
        <tr>
            <td>基数排序</td>
            <td>$O(d(n+r))$</td>
            ...
            <td>是</td>
            <td>顺序存储,链式存储</td>
            <td>时间复杂度和初始序列无关,和d相关(关键字位数)</td>
            <td>/</td>
            <td>n较大,关键字位数少且可以分解,关键字是整数</td>
        </tr>
        <tr>
            <td>计数排序</td>
            <td>/</td>
...
查询效率
* 有序顺序表:$O(log_{2}n)$
* 二叉排序树:最好的情况是平衡二叉树$O(log_{2}n)$,最坏的情况是一串$O(n)$
* 堆:最坏的情况$O(n)$,即顺序遍历顺序表
* 平衡二叉树:$O(log_{2}n)$

注意
* 时间复杂度线性,指的是$O(n)$
* 冒泡排序常常和直接插入排序对比
* 选择排序算法需要考虑:时间复杂度,空间复杂度,数据规模,数据存储方式,算法稳定性,数据初始状态

### 外部排序

##### 概念

* 外存,内存之间的数据交换:操作系统按照块进行磁盘存储的管理;读写磁盘;缓冲区
* 外部排序:使用归并排序方法,最少只需要在内存中分配3块缓冲区即可进行对文件的排序
  * 包含两个输入缓冲区和一个输出缓冲区
  * 两个输入缓冲区读取的文件内容,要进行内部排序(例如归并排序)
  * 通过输出缓冲区,写回磁盘,构建初始归并段
  * 进行归并(也是基于内存的内部归并算法),输出缓冲区满则写出磁盘
  * 注意输入缓冲区空也会立即写入数据
  * 不断进行归并直到最终排序结束
* 时间开销:读写外存的时间(大部分)+内部排序时间(例如归并排序)+内部归并时间
* 读写外存的时间基本上等同于读写磁盘次数(秒级别)
* 缓冲区不能同时写入写出,所以并行操作的话需要额外增加缓冲区

优化
* 多路归并,减少趟数,从而减少读写磁盘次数
  * r个初始归并段,k叉树,高度h:最少归并趟数$\lceil log_{k}r \rceil$
  * 但是k增大也会导致内存缓冲区的增长,以及关键字对比次数的增加(k-1)
  * 败者树解决(减少关键字对比次数)
  * 生成初始归并段越长,那么r越少,从而减少读写磁盘的次数
  * 置换-选择排序解决(减少初始归并段数量)
* 多路归并排序:$\lceil \frac{n}{k^t} \rceil = 1$,其中n代表元素个数,k代表多路归并的路数,t代表排序趟数

##### 败者树

* 减少关键字对比次数(k-1),从而减少内部归并时间
* 败者树:理解为完全二叉树,多了一个根节点头部,k个叶子节点代表比较的关键字,非叶子节点记录比较失败的关键字,胜者一直向上直到根节点
* 败者树的每个叶子节点对应一个初始归并段,每个非叶子节点记录失败的(或者根部那个最终成功的)关键字来自于哪个初始归并段,构建败者树;获取到冠军后,对应的初始归并段补充下一个元素继续参与败者树进行比较
* k路归并,第一次构建败者树,需要关键字对比次数k-1,后续只需对比最多$\lceil log_{2}k \rceil$
* 代码思路:k路归并,需要长度k的数组,索引0存储冠军归并段编号,

##### 置换-选择排序

* 减少初始归并段数量,从而减少磁盘读写次数
* 每次输出最小/最大的元素到初始归并段,然后使用变量记录最小/最大值,然后继续读取继续比较,直到全部都比记录的变量值小/大,这个归并段才结束;这样得到的归并段长度可以超出内存操作区限制,但是每个归并段长度不再相等
* 注意读取和输出,都是到缓冲区,操作系统读写磁盘仍然是按照块为单位到缓冲区中操作的
* n个记录的文件,置换-选择排序产生m个归并段,m和n没有关系

##### 最佳归并树

* 初始归并段长度不同(是因为置换-选择排序导致),可能包含了多个块大小的元素,在归并的过程中,磁盘读/写总次数=归并树的WPL*2;让这个总次数最少,实际上就是构建哈夫曼树
* 对于多路归并,也是参考哈夫曼树实现原理进行构建,如果归并段数量不够,那么设置长度0的虚拟段
* 判断归并段数量是否足够:
  * $n_0=初始归并段数量+虚拟段数量$,即为度为0的节点数量
  * $n_k=\frac{n_0 -1}{k-1}$,即为度为k的节点数量
  * 可以计算$(初始归并段数量-1)\%(k-1)$的值,有余数u的话,需要补充虚拟段$(k-1)-u$
* 最佳归并树:实际上就是k路归并的哈夫曼树的构建